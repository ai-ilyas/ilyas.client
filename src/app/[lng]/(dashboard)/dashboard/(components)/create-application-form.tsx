...
import { AlertDialog, AlertDialogCancel, AlertDialogContent, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger } from '@/src/lib/presenter/components/ui/alert-dialog';
...
const createApplicationForm: React.FC<CreateApplicationFormProps> = ({ apps, lng }) => {  
  ...
  const defaultValues = { applicationName: '' };
  ...
  const onSubmit = async (data: ApplicationFormValues) => {
    ...
    try{
      ...
      if (response.status === 500 && queryResponseText.includes("Duplicate name"))
      {
        ...
        toast({
          ...
          description: t("dashboard_home_toast_applicationNameAlreadyExists", { applicationName: data.applicationName })
        });
        /* */
      } 
    ...
    catch(error)
    {
      ...
      toast({
        variant: 'destructive',
        title: t("common_toast_error_title"),
        description: t("dashboard_home_toast_errorOccuredDuringCreationProcess", { applicationName: data.applicationName })
      });
      setLoading(false);
    }
  };
  ...
  return (
    <AlertDialog>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-2xl font-medium">
              {t("dashboard_home_newApplication")}
            </CardTitle>
            <svg
          ...
          <CardContent>                  
            <p className="text-xs text-left text-muted-foreground">
              {t("dashboard_home_createAnEmptyApplication")}
            </p>
          </CardContent>
        </Card>
      </AlertDialogTrigger>
      <AlertDialogContent>
      <Form {...form}>
        <form action={postUrl} method='POST' onSubmit={form.handleSubmit(onSubmit)}>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("dashboard_home_createANewApplication")}</AlertDialogTitle>
                <div className="gap-8">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t("dashboard_home_applicationName")}</FormLabel>
                        <FormControl>
                          <Input 
                            disabled={loading}
                            placeholder={t("dashboard_home_applicationName")}
                            {...field}
                          />
                        </FormControl>
                        <FormMessage />
          ...
          <AlertDialogFooter className='mt-4'>
            ...
            <LoadingButton loading={loading} type="submit" text={t("common_create")}>
