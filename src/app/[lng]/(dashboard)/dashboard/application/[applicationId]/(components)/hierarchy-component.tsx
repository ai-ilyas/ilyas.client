...
const hierarchyComponent: React.FC<HierarchyProps> = ({ app, lng, apps }) => {
  const { t } = useTranslation(lng);
  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle>
          <Network className="inline-block h-4 w-4" />{' '}
          {t('application_hierarchy_hierarchy')}
        </CardTitle>
        <CardDescription>
      ...
      <CardContent>
        <h4 className="mb-2 text-sm font-medium">
          {t('application_hierarchy_parentApplication')}
        </h4>
        <ParentApplication app={app} lng={lng} apps={apps}></ParentApplication>
        <Separator className="my-4" />
        <div className="space-y-4">
          <h4 className="text-sm font-medium">
            {t('application_hierarchy_childrenApplication')}
          </h4>
