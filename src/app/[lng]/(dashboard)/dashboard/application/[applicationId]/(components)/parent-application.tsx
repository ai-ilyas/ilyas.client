...
const parentApplication: React.FC<ParentApplicationFormProps> = ({
  ...
  const updateParentApplication = useMutation(
    api.applications.updateParentApplication
  );
  ...
  const onRemoveParentApplication = async () => {
    ...
    try {
      ...
      toast({
        title: t(
          `application_parentApplication_parentApplicationHasBeenRemoved`
        ),
        variant: 'default'
      });
    } catch (error) {
      ...
      toast({
        title: t(
          `application_parentApplication_parentApplicationHasNotBeenRemoved`
        ),
        variant: 'destructive'
      });
      throw error;
      ...
      <>
        {app.parentApplicationId && (
          <div className="flex justify-between font-extrabold">
            <div>
              <Link href={'/dashboard/application/' + app.parentApplicationId}>
              ...
              <Dialog open={openRemove} onOpenChange={setOpenRemove}>
                ...
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>
                      {t(
                        `application_parentApplication_removeTheParentApplication`
                      )}
                    </DialogTitle>
                    <div className="text-sm text-muted-foreground">
                      {t(
                        `application_parentApplication_areYouSureYouWantToRemoveTheParentApplication`
                      )}
                    </div>
                  </DialogHeader>
                  <DialogFooter>
                    ...
                    <Button
                      onClick={() => onRemoveParentApplication()}
                      variant="destructive"
                    >
                      {t('common_remove')}
                    </Button>
          ...
          <DialogContent className="sm:max-w-[425px]">
            <Form {...form}>
              <form onSubmit={form.handleSubmit((data) => onSubmit(data))}>
                <DialogHeader>
                  <DialogTitle>
                    {t(`application_parentApplication_addParentApplication`)}
                  </DialogTitle>
                </DialogHeader>
                <div className="mt-3 gap-8">
                  <DialogDescription className="mb-2">
                    {t(
                      `application_parentApplication_selectExistingParentApplication`
                    )}
                  </DialogDescription>
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-col">
                        <FormLabel>
                          {t(`application_parentApplication_parentApplication`)}
                        </FormLabel>
                        <Popover
                          open={openPopover}
                        ...
                        >
                          <PopoverTrigger asChild>
                            <FormControl>
                              ...
                              >
                                {field.value
                                  ? apps.find((x) => x._id === field.value)
                                      ?.name
                                  : t('common_select', {
                                      value: t(`common_application`)
                                    })}
                  ...
                  <DialogDescription>
                    {t(
                      `application_parentApplication_createANewParentApplication`
                    )}
                  </DialogDescription>
                  <FormField
                    control={form.control}
