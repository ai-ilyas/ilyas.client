...
const childrenApplications: React.FC<ChildrenApplicationsProps> = ({
  ...
  const addChildrenApplications = useMutation(
    api.applications.addChildrenApplications
  );
  const removeParentApplication = useMutation(
    api.applications.removeParentApplication
  );
  const [openRemove, setOpenRemove] = useState(false);
  const [childAppToRemove, setChildAppToRemove] = useState<IApplication>();
  const childApplications = apps.filter(
    (x) => x.parentApplicationId === app._id
  );
  ...
  const onRemoveChildApplication = async () => {
    ...
    try {
      ...
      toast({
        title: t(
          `application_childrenApplication_childApplicationHasBeenRemoved`
        ),
        variant: 'default'
      });
    } catch (error) {
      ...
      toast({
        title: t(
          `application_childrenApplication_childApplicationHasNotBeenRemoved`
        ),
        variant: 'destructive'
      });
      throw error;
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <p className="text-sm font-medium leading-none">
                      <Link href={'/dashboard/application/' + childApp._id}>
                        {childApp.name}
                      </Link>
                    </p>
                    <p className="text-sm text-muted-foreground">
                      {childApp.description &&
                        childApp.description?.substring(0, 25) +
                          (childApp.description &&
                            childApp.description.length > 25 &&
                            '...')}
                    </p>
                  </div>
                </div>
                <Dialog open={openRemove} onOpenChange={setOpenRemove}>
                  <DialogTrigger asChild>
                    <Button
                      onClick={() => setChildAppToRemove(childApp)}
                      variant={'destructive'}
                      className="shrink-0"
                    >
                  ...
                  <DialogContent>
                    <DialogHeader>
                      <DialogTitle>
                        {t(
                          `application_childrenApplication_removeChildApplication`
                        )}
                      </DialogTitle>
                      <div className="text-sm text-muted-foreground">
                        {t(
                          `application_childrenApplication_areYouSureYouWantToRemoveThisChildApplication`
                        )}
                        <div className="db text-center font-black">
                          {childAppToRemove?.name}
                        </div>
                      </div>
                    </DialogHeader>
                    <DialogFooter>
                      ...
                      <Button
                        onClick={() => onRemoveChildApplication()}
                        variant="destructive"
                      >
                        {t('common_remove')}
                      </Button>
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit((data) => onSubmit(data))}>
                  <DialogHeader>
                    <DialogTitle>
                      {t(
                        `application_childrenApplication_addChildrenApplication`
                      )}
                    </DialogTitle>
                  </DialogHeader>
                  <div className="mt-3 gap-8">
                    <DialogDescription className="mb-2">
                      {t(
                        `application_childrenApplication_selectExistingChildrenApplication`
                      )}
                    </DialogDescription>
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="flex flex-col">
                          <FormLabel>
                            {t(
                              `application_childrenApplication_childrenApplication`
                            )}
                          </FormLabel>
                          <MultiSelect
                            lng={lng}
                            ...
                            options={apps
                              .filter(
                                (x) =>
                                  x.parentApplicationId !== app._id &&
                                  app._id !== x._id &&
                                  x._id !== app.parentApplicationId
                              )
                              .map((x) => ({ value: x._id, label: x.name }))}
                            onValueChange={setSelectedChildren}
                            defaultValue={selectedChildren}
                            placeholder={t(
                              'application_childrenApplication_selectChildrenApplication'
                            )}
                            variant="secondary"
                            animation={2}
                            maxCount={3}
                    ...
                    <DialogDescription>
                      {t(
                        `application_childrenApplication_createANewChildApplication`
                      )}
                    </DialogDescription>
                    <FormField
                      control={form.control}
                          ...
                          <FormControl>
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                setIsDescriptionDisabled(
                                  e.target.value === '' ? true : false
                                );
                                if (e.target.value)
                                  form.resetField('parentApplicationId');
                              }}
                            />
                          </FormControl>
                          <FormMessage />
