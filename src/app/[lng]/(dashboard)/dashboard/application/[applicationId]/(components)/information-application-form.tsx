...
const informationApplicationForm: React.FC<UpdateApplicationFormProps> = ({
  ...
  const onSubmit = async (data: ApplicationFormValues) => {
    ...
    try {
      ...
      if (
        response.status === 500 &&
        queryResponseText.includes('Duplicate name')
      ) {
        ...
        toast({
          ...
          description: t('dashboard_home_toast_applicationNameAlreadyExists', {
            applicationName: data.applicationName
          })
        });
        /* */
      } else if (response.status !== 200) {
        throw new Error('Problem during Application update process.');
      }
      else{
        toast({
            description: "Saved !"
      ...
      toast({
        ...
        title: t('common_toast_error_title'),
        description: t(
          'dashboard_home_toast_errorOccuredDuringCreationProcess',
          { applicationName: data.applicationName }
        )
      });
      setLoading(false);
