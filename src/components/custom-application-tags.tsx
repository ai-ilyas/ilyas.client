...
export default function customApplicationTags({
  lng,
  tags,
  applicationId,
  type,
  ...
  const formSchema = z
    .object({
      ...
      color: z
        ...
        .refine(
          (val) =>
            !availableTags?.some((x) => x.value === val && x.color === hex),
          {
  ...
  useEffect(() => {
    maxNumber && linkedTags?.length && linkedTags?.length >= maxNumber
  ...
  const getBadge = (tag: ITag | undefined) =>
    tag && <Badge style={{ background: tag.color }}>{tag.value}</Badge>;
  return (
    <div>
      {linkedTags?.map((x) => (
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <div className="text-sm text-muted-foreground">
                  ...
                  <Badge
                    style={{ background: tagToRemove?.color }}
                    className="m-0.5"
                  >
                    {tagToRemove?.value}
                  </Badge>
              ...
              <DialogFooter>
                ...
                <Button onClick={() => onRemoveTag()} variant="destructive">
                  {t('common_remove')}
                </Button>
              </DialogFooter>
            </DialogContent>
      ...
      <Dialog modal={true} open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
          <Button
            ...
            variant="link"
          >
            <IconAdd className="inline-block h-4 w-4"></IconAdd>
            {t(`custom${labelType}_add`)}
          </Button>
        </DialogTrigger>
                          ...
                          <FormControl>
                            ...
                            >
                              {field.value
                                ? getBadge(
                                    availableTags?.find(
                                      (tag: ITag) => tag._id === field.value
                                    )
                                  )
                                : t('common_select', {
                                    value: t(`custom${singleLabelType}`)
                                  })}
