...
export function Prices({ props: prices }: { props: Price[] }) {
  const sumPrices = prices.reduce((total, item) => total + item.monthly_cost, 0);
      ...
      <Table>
        ...
        {prices.map(price => (
                    <TableRow key={price.service_name}>
                      ...
                      <TableCell className="text-right">{price.price_per_unit}</TableCell>
                      <TableCell>{price.unit}</TableCell>
                      <TableCell>{price.quantity}</TableCell>
                      <TableCell className="text-right">{price.monthly_cost} $</TableCell>
                    </TableRow>
                  ))}
