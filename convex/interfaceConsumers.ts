...
export const list = query({
  ...
  handler: async (ctx, { applicationId }) => {
    ...
    if (application?.userId !== userId)
      throw new Error('InterfaceConsumers.list - Method not allowed.');

    const interfaceConsumers = await ctx.db
      .query(INTERFACE_CONSUMERS_TABLE)
      .withIndex('byApplicationId', (q) => q.eq('applicationId', applicationId))
      .collect();

    return interfaceConsumers.sort((a, b) => b.editionTime - a.editionTime);
  }
});
...
export const insert = mutation({
  ...
  handler: async (
    ctx,
    { direction, volumetry, applicationId, frequence, interfaceId }
  ) => {
    ...
    if (application.userId !== userId)
      throw new Error('InterfaceConsumers.insert - Method not allowed.');
...
export const patch = mutation({
  ...
  handler: async (ctx, { _id, direction, volumetry, frequence }) => {
    ...
    await validateInterfaceConsumer(
      ctx,
      direction,
      _interfaceConsumer.interfaceId
    );
...
async function validateInterfaceConsumer(
  ...
  if (
    _interface!.direction !== 'bi-directional' &&
    _interface!.direction === direction
  )
    throw new Error("interfacesConsumers.insert - Direction doesn't match.");
}
