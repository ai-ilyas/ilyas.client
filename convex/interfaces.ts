...
export const findOne = query({
  ...
  handler: async (ctx, { id }) => {
    ...
    // #010 SERVER Get interface only when current userId match with userId in Application Table
    if (interface_?.userId !== userId)
      throw new Error('interfaces.findOne - Method not allowed.');
...
export const insert = mutation({
  ...
  handler: async (
    ...
    await validateInterface(
      ...
      userId,
      applicationId,
      itComponentId as Id<'tags'>,
      dataObjectId as Id<'tags'>
    );
    ...
    const interfaceId = await ctx.db.insert(INTERFACES_TABLE, {
      ...
      description,
      direction,
      itComponentId: itComponentId as Id<'tags'>,
      dataObjectId: dataObjectId as Id<'tags'>,
      volumetry,
      applicationId,
      frequence,
      userId,
...
export const removeInterface = mutation({
  ...
  handler: async (ctx, { _id }) => {
    ...
    if (userId !== interfaceToRemove?.userId)
      throw new Error('interfaces.removeInterface - Method not allowed.');
...
async function validateInterface(
  ...
  const existingInterface = await ctx.db
    .query(INTERFACES_TABLE)
    .withIndex('byNameApplicationId', (q) =>
      q
        .eq('name', name)
  ...
  if (existingInterface)
    throw new Error('interfaces.insert - Name already used.');
