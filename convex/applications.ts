...
export const list = query({
  ...
  handler: async (ctx) => {
    ...
    console.log(userId);
    const applications = await ctx.db.query("applications").withIndex("byUserId", (q) => q.eq("userId", userId!)).collect();
...
export const patch = mutation({
  args: { _id: v.id(TABLENAME), name: v.string(), description: v.string() },
  handler: async (ctx, { _id, name, description }) => {
    ...
    if (userId !== applicationToUpdate?.userId) throw new Error("Operation not allowed.")
