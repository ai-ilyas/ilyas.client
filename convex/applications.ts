...
export const findOne = query({
  ...
  handler: async (ctx, { id }) => {
    ...
    if (application?.userId !== userId)
      throw new Error('applications.findOne - Method not allowed.');
    ...
    const interfaces = await ctx.db
      ...
      .withIndex('byApplicationId', (q) =>
        q.eq('applicationId', _id).eq('userId', userId!)
      )
      .collect();

    const itComponentIds = interfaces
      .filter((x) => x.itComponentId)
      .map((x) => x.itComponentId!);
    const dataObjectIds = interfaces
      .filter((x) => x.dataObjectId)
      .map((x) => x.dataObjectId!);
    ...
    return {
      ...
      interfaces: interfaces.map((x) => {
        const dataObjectTag = dataObjectTags.find(
          (y) => y?._id === x.dataObjectId
        );
        const itComponentTag = itComponentTags.find(
          (y) => y?._id === x.itComponentId
        );
...
export const insert = mutation({
  ...
  handler: async (ctx, { name }) => {
    ...
    const existingApp = await ctx.db
      .query(APPLICATIONS_TABLE)
      .withIndex('byName', (q) => q.eq('name', name).eq('userId', userId!))
      .first();
    if (existingApp)
      throw new Error('applications.insert - Name already used.');

    // #030 SERVER When Insert application current userId is the userId in Application Table
    const appId = await ctx.db.insert(APPLICATIONS_TABLE, {
      name,
      userId,
...
export const patch = mutation({
  ...
  handler: async (
    ctx,
    { _id, name, description, businessOwner, technicalOwner, numberOfUsers }
  ) => {
    // #040 CLIENT SERVER Application name length should be between 3 and 50 characters
    checkIfStringIsNotOutOfLimits(name, { min: 3, max: 50 });
    ...
    if (userId !== applicationToUpdate?.userId)
      throw new Error('applications.patch - Method not allowed.');
...
export const updateParentApplication = mutation({
  ...
  handler: async (ctx, { _id, parentApplicationId, name, description }) => {
    ...
    if (_id === parentApplicationId)
      throw new Error(
        'applications.updateParentApplication - 80 Application cannot be its own parent application.'
      );

    // #090 CLIENT SERVER Application cannot be parent and child for another application
    const parentApplication = await ctx.db.get(
      parentApplicationId as Id<'applications'>
    );
    if (_id === parentApplication?.parentApplicationId)
      throw new Error(
        'applications.updateParentApplication - 90 Application cannot be its own parent application.'
      );
    ...
    if (userId !== applicationToUpdate?.userId)
      throw new Error(
        'applications.updateParentApplication - Method not allowed.'
      );
    ...
    if (parentApplicationId) {
      ...
      if (!name)
        throw new Error(
          'applications.updateParentApplication - Name cannot be null.'
        );
      ...
      // #030 SERVER When Insert application current userId is the userId in Application Table
      _parentApplicationId = await ctx.db.insert(APPLICATIONS_TABLE, {
        name: name!,
        userId,
        editionTime: Date.now()
      });
    ...
    await ctx.db.patch(_id, {
      parentApplicationId: _parentApplicationId,
      editionTime: Date.now()
    });
  }
});
...
export const removeParentApplication = mutation({
  ...
  handler: async (ctx, { _id }) => {
    ...
    if (userId !== applicationToUpdate?.userId)
      throw new Error(
        'applications.removeParentApplication - Method not allowed.'
      );
...
export const addChildrenApplications = mutation({
  ...
  handler: async (
    ctx,
    { parentApplicationId, childrenApplicationsIds, name, description }
  ) => {
    const userId = (await getUserId(ctx, true))!;
    const parentApplication = await ctx.db.get(parentApplicationId);

    // #020 SERVER Insert or Update application only when current userId match with userId in Application Table
    if (userId !== parentApplication?.userId)
      throw new Error(
        'applications.addChildrenApplications - Method not allowed.'
      );
    ...
    if (childrenApplicationsIds) {
      ...
      // #090 CLIENT SERVER Application cannot be parent and child for another application
      if (
        childrenApplicationsIds.some(
          (x) =>
            parentApplicationId === (x as Id<'applications'>) ||
            parentApplication.parentApplicationId === x
        )
      )
        throw new Error(
          'applications.updateParentApplication - Application cannot be its own parent application.'
        );

      const applicationsToUpdate = await getAll(
        ctx.db,
        childrenApplicationsIds.map((x) => x as Id<'applications'>)
      );
      // #020 SERVER Insert or Update application only when current userId match with userId in Application Table
      if (applicationsToUpdate.some((x) => x?.userId !== userId))
        throw new Error(
          'applications.addChildrenApplications - Method not allowed.'
        );
    }
    ...
    if (childrenApplicationsIds && childrenApplicationsIds.length > 0) {
      childrenApplicationsIds.map(
        async (x) =>
          await ctx.db.patch(x as Id<'applications'>, {
            parentApplicationId,
            editionTime: Date.now()
          })
      );
    ...
    if (name) {
      // #030 SERVER When Insert application current userId is the userId in Application Table
      await ctx.db.insert(APPLICATIONS_TABLE, {
        name: name!,
        userId,
        parentApplicationId,
        editionTime: Date.now()
