...
export const list = query({
  ...
  handler: async (ctx, { type }) => {
    ...
    const tags = await ctx.db
      .query(TAGS_TABLE)
      .withIndex('byType', (q) => q.eq('userId', userId!).eq('type', type))
      .collect();
...
export const insert = mutation({
  ...
  handler: async (
    ctx,
    { value, type, color, icon, description, applicationId }
  ) => {
    ...
    const tagId = await ctx.db.insert(TAGS_TABLE, {
      value: value.trim(),
      type,
      color,
      userId,
    ...
    if (applicationId) {
      await validateAndLinkToApplication(
        ctx,
        applicationId,
        userId,
        tagId,
...
export const patch = mutation({
  ...
  handler: async (ctx, { _id, value, color, icon, description }) => {
    ...
    if (userId !== tagToUpdate?.userId)
      throw new Error('tags.patch - Method not allowed.');
...
export const linkToApplication = mutation({
  ...
  handler: async (ctx, { tagId, applicationId }) => {
    ...
    // #020 SERVER Update tag only when current userId match with userId in Tag Table
    if (userId !== tagToAdd?.userId)
      throw new Error('tags.linkToApplication - Method not allowed.');

    const tagToUpdate = await ctx.db.get(tagId as Id<'tags'>);
    validateAndLinkToApplication(
      ctx,
      applicationId,
      userId,
      tagId as Id<'tags'>,
...
export const deleteTag = mutation({
  ...
  handler: async (ctx, { id }) => {
    ...
    if (userId !== tagToDelete?.userId)
      throw new Error('tags.deleteTag - Method not allowed.');
...
export const removeLinkToApplication = mutation({
  ...
  handler: async (ctx, { tagId, applicationId }) => {
    ...
    if (userId !== application?.userId)
      throw new Error('tags.deleteTag - Method not allowed.');
    const appTag = await ctx.db
      .query(APPLICATION_TAGS_TABLE)
      .withIndex('byApplicationId', (q) => q.eq('applicationId', applicationId))
...
async function validateAndLinkToApplication(
  ...
  if (userId !== applicationToUpdate?.userId)
    throw new Error('tags.insert - Method not allowed.');
  const appTags = await ctx.db
    .query(APPLICATION_TAGS_TABLE)
    .withIndex('byApplicationId', (q) => q.eq('applicationId', applicationId))
    .collect();
  ...
  if (
    appTags.filter((x) => tags.find((y) => y?._id === x.tagId)?.type === type)
      .length >= MAX_TAG_NUMBER
  )
    throw new Error('tags.linkToApplication - Too many tags already added.');
  if (appTags.some((x) => x.tagId === (tagId as Id<'tags'>)))
    throw new Error(
      'tags.linkToApplication - Tag already exists with this application.'
    );
...
function validateTag(
  ...
  if (value.trim() === '')
    throw new Error('Impossible to add a tag with no value.');
  // #060 CLIENT SERVER Tag name length should be 50 characters maximum
  if (value.length > 50)
    throw new Error('Impossible to add a tag with more than 15 characters.');
  // #0110 CLIENT SERVER Tag description length should be 500 characters maximum
  if (description && description.length > 500)
    throw new Error('Impossible to add a tag with more than 500 characters.');
  // #080 CLIENT SERVER Tag color should be a valid HTML hexadecimal color
  if (color && !isValidHtmlColor(color))
    throw new Error('Impossible to add a tag with this color.');
}
