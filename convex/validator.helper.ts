...
export const checkIfStringIsNotOutOfLimits = (
  stringToTest?: string,
  { min, max, nullAllowed = false }: CheckStringOptions = {}
) => {
...
export const checkUserIdsMatch = (
  userId: string,
  userId2: string,
  fromMethod?: string
) => {
  if (userId !== userId2)
    throw new Error((fromMethod + ' - ' ?? ' ') + "userId doesn't match.");
};
